...
class Prior:
    ...
    class AllLats:
        def __init__(self,D):
            from numpy import empty   
            self.q=empty( (D.nR,D.nt-1) )  #lateral inflows [m^2/s]
            self.qv=empty( (D.nR* (D.nt-1),1 )  )            #N*1
                
class Truth:
    def __init__(self,D):
        from numpy import empty 
        self.A0=[] #cross-sectional area for first time index [m^2] (1,D.nR)
...
class SoS_calibration:
    def __init__(self,DAll):
        from numpy import empty
        self.sosA0=empty((DAll.nR,1))
        self.sosna=empty((DAll.nR,1))
        self.sosx1=empty((DAll.nR,1))
        self.gaugetime=[]
        self.gaugedisg=[]
        self.product_time=[]
        self.product_disg=[]
