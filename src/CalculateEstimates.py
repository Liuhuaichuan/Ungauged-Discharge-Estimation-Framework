...
import numpy as np
NaN=np.nan
from numpy import mean,cov,sqrt,diagonal,empty,ones,std,diag,log,corrcoef
...
def CalculateEstimates(C,D,Obs,Prior,DAll,AllObs,nOpt):
    ...
    E=Estimates(D,DAll)
    E.A0hat=mean(C.thetaA0[:,C.Nburn+1:C.N],1) #axis=1
    E.A0hat_peak=np.array([find_peak_position(C.thetaA0[i,C.Nburn+1:C.N]) for i in range(0,D.nR)])# Get peak position for A0
    E.CA0=cov(C.thetaA0[:,C.Nburn+1:C.N])# n.DR*n.DR
    E.stdA0Post=sqrt(diagonal(E.CA0))
    
    E.nahat=mean(C.thetana[:,C.Nburn+1:C.N],1)#nR*1    
    ...
    E.nhatAll_peak=empty([D.nR,DAll.nt])# A0 peak for total period
    for i in range(0,D.nR):
        ...
        E.nhatAll_peak[i,:]=calcnhat(AllObs.w[i,:], AllObs.h[i,:], AllObs.hmin[i], E.A0hat_peak[i]+AllObs.dA[i,:]-AllObs.A0Shift[i], E.x1hat[i], E.nahat[i], nOpt)
    ...
    for i in range(0,C.N):
        for r in range(0,D.nR):
            ...
            nhatAll[r,:]=calcnhat(AllObs.w[r,:], AllObs.h[r,:], AllObs.hmin[r], \
                               E.A0hat[r]+AllObs.dA[r,:]-AllObs.A0Shift[r], \
                               C.thetax1[r,i],C.thetana[r,i],nOpt)
    ...
    E.QhatPost=mean(C.thetaQ[C.Nburn:,:,:],0)# D.nR × D.nt
    ...
    nhat=empty([D.nR,DAll.nt])# [D.nR,DAll.nt]
    nhat[:]=NaN
    for r in range(0,D.nR):
    ...
    if nOpt==3:
        ...
        E.QhatUnc_A0=((5/3-x1) * sigA0/A)**2#∂Q/∂A0*A0/Q
    ...
    #5)all estimates
    E.AllQ_peak=empty([D.nR,DAll.nt])
    for i in range(0,D.nR):
        E.AllQ[i,:]=1/E.nhatAll[i,:]*(E.A0hat[i]+AllObs.dA[i,:]-AllObs.A0Shift[i])**(5/3) * AllObs.w[i,:]**(-2/3) * AllObs.S[i,:]**0.5
        E.AllQ_peak[i,:]=1/E.nhatAll_peak[i,:]*(E.A0hat_peak[i]+AllObs.dA[i,:]-AllObs.A0Shift[i])**(5/3) * AllObs.w[i,:]**(-2/3) * AllObs.S[i,:]**0.5
