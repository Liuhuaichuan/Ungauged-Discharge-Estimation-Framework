...
def MakeFigs(D,Truth,Prior,C,E,Err,AllTruth,DAll,AllObs,nOpt):
    ...
    for i in range(0,D.nR):
        ...
        y=lognorm.pdf(x,sigmana[i],0,exp(muna[i]))
        plt.plot(x,y/max(y)*C.N/20,'r--',linewidth=2)
    ...
    vx1=(covx1*meanx1)**2
    [mux1,sigmax1] = logninvstat(abs(meanx1),vx1)
    
    for i in range(0,D.nR):
        plt.subplot(1,D.nR,i+1)
        x=linspace(min(C.thetax1[i,:]),max(C.thetax1[i,:]),100) 
        if nOpt<5:
            y=lognorm.pdf(-x,sigmax1[i],0,exp(mux1[i]))
        else:
            y=lognorm.pdf(x,sigmax1[i],0,exp(mux1[i]))
        plt.plot(x,y/max(y)*C.N/20,'r--',linewidth=2)
    ...
    plt.figure(10)
    plt.plot(range(0,D.nR),Err.QRelErrPrior,'.-',label='Prior')#先验算出来流量误差
