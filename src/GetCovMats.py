...
def GetCovMats(D,Obs,Prior):
    ...
    DeltaA=CalcADelta(D.nR,D.nt)# N*M
    B=CalcB(D.nR,D.nt)# N*M

    stop=1
    
    Obs.Ch=Obs.sigh**2 * eye(M)
    Obs.Cw=Obs.sigw**2 * eye(M)
    Obs.CS=Obs.sigS**2 * eye(M)# M*M
    
    # calculate Jacobian of the dAdt term wrt h and w， Obs.wv，M*1
    Obs.JAh=(B @ Obs.wv @ ones((1,M)))*DeltaA/(D.dt @ ones((1,M)))
    Obs.JAw=(DeltaA @ Obs.hv @ ones((1,M)) ) * B / (D.dt @ ones((1,M)))
    Obs.JA=concatenate((Obs.JAh,Obs.JAw),1)#N*2M,[Obs.JAh,Obs.JAw]
    ...
    Obs.CSdAw=concatenate( (concatenate((Obs.CS,zeros((M,M)),zeros((M,M))),1),\
                            ...
                            concatenate((zeros((M,M)),zeros((M,M)),Obs.Cw),1)),0) #3M*3M
